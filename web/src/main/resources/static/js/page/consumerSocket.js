...
var consumerSocket = {
	...
	socket:function(){
		...
		if ('WebSocket' in window) {
			_websocket = new WebSocket("ws://localhost:8980/js/websocket/consumer/xiaobai/799999999");
		} else {
	...
	adReply : function(message){
		...
		$("#msgShow").append("<div style='text-align: center;'><div style='font-size: 6px; background-color: #dcdada; width: 180px; margin: auto; color: white; border-radius: 1em 1em; padding: 4px;'>"+new Date().Format("yyyy-MM-dd hh:mm:ss")+"Start communication</div></div>");
	},
	send : function(){
		...
		if(editor.txt.html().trim()!=""){
			...
			console.log("用户发送消息："+sendObj.html());
